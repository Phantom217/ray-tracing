...
impl Aabb {
    ...
    pub fn hit(&self, ray: &Ray, mut t_range: std::ops::Range<f64>) -> bool {
        use crate::vec3::Axis::*;
    ...
    // experimental vector rewrite of the above algorithm
    pub fn hit_v(&self, ray: &Ray, mut t_range: std::ops::Range<f64>) -> bool {
        ...
        t_range.start = t_range.start.max(t0_.reduce(f64::max));
        t_range.end = t_range.end.max(t1_.reduce(f64::min));
